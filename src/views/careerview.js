...
ET.CareerView = Backbone.View.extend({
	...
	initialize : function() {
		...
		this.listenTo(ET.mapView.model, "change:viewType", this.renderMenu);
	},
	/**
	...
	renderMenu : function() {
		...
		var activeTabIndex = $('#sidebar-menu .nav-tabs').find('.active').index();
		//PROBLEM: if you switch to another view (region -> city), the old tab index will be remembered

		switch(appView.model.get('viewType')) {
			case ViewTypes.Nation:
				var region = appView.model.get('region');
				$('#sidebar-menu').html(this.menuNationalTemplate({
					stats : this.model,
					cities : cities
				}));
				break;
			case ViewTypes.Region:
				var region = appView.model.get('region');
				$('#sidebar-menu').html(this.menuRegionalTemplate({
					stats : this.model,
					region : region,
					cities : cities
				}));
			...
			case ViewTypes.City:
				...
				$('#sidebar-menu').html(this.menuCityTemplate({
					city : city
				}));
		}

		//re-activate that tab
		$('#sidebar-menu .nav-tabs').find(_.format('li:eq(<%=index%>) a', {
